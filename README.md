...
打开 `http://localhost:8080`。图渲染用的 Cytoscape 已经 vendor 进 `src/main/resources/static/vendor/`（见该目录 `README.md` 的版本与来源），**没有 CDN、没有 npm 构建步骤**，内网离线可用。布局不分给 dagre：表级图由 `js/graph.js` 拿服务端下发的 `layer` 当列号自算坐标，并在 LR / TB 两个方向里挑放缩更大的那个；字段级图更彻底——**盒子和字段行的 `x/y/width/height` 连同字号字体都由服务端算好下发**（`js/sqlflowView.js` 只做左上角 → 中心点的换算），所以"这张图长什么样"是能在 Java 单测里断言的，前端也没有第二份会走样的尺寸口径。

画布窄到放不下整图时**优先保字不保图**：缩放停在 10px 可读标签的下限上、从链路起点开始摆，被裁掉的那一头拖动画布能看到，右下角同时写明"画布 WxH 放不下 N 个节点"。为什么这么取舍，见方案 §8.9。

字段级血缘是**一张表一个盒子、盒子里一行一个字段、线连字段行不连盒子**：CTE / 子查询 / UNNEST 折掉的中间关系不再只藏在边的 `hops` 里，而是摊成虚线暗盒画在上下游之间，一条逻辑边拆成几段，点任意一段仍然打开原边的逐跳证据。**点一个字段行会在它旁边弹出跟随式动作条**：`只看这一列`（纯客户端沿边收出通路、盒子按留下的行数缩，横向一层一列一律不动）/ `列出字段`（右栏出该实体全量清单，宽表在这里看得全）/ `复位`，`×` 或点画布空白收起；点盒顶的表名则以那张表为中心重新展开。783 列那种宽表不再拒绝渲染：每盒只画有边的前若干列，剩下的折成一行"还有 N 列未画"，超出登记上限的列数与因两端没画上而暂时缺的段数一起由 `metaInfo.warning` 说清。取舍与实测见方案 §8.10、§8.11、§8.12。
...
src/main/java/com/bigdata/lineage/
  ...
  web/sqlflow/                                     字段级血缘的对方形状：数据模型 + 关系 + 服务端布局（见方案 §8.11）
  tools/SqlDirLineageTool.java                     目录批量解析 CLI
src/main/resources/static/                         前端：无框架、无打包，ES module 直出；字段级图只照抄服务端坐标
