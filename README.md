...
```bash
mvn -o clean package             # 287 用例 + 可执行 jar
...
打开 `http://localhost:8080`。图渲染用的 Cytoscape 已经 vendor 进 `src/main/resources/static/vendor/`（见该目录 `README.md` 的版本与来源），**没有 CDN、没有 npm 构建步骤**，内网离线可用。布局不分给 dagre：表级图由 `js/graph.js` 拿服务端下发的 `layer` 当列号自算坐标，并在 LR / TB 两个方向里挑放缩更大的那个；字段级图连 Cytoscape 都不用了——**盒子和字段行是 DOM，线是一层 SVG**，服务端只给网格（`layer` = 第几层、`slot` = 同层第几个），像素由浏览器量（`js/sqlflowView.js`）。分工是刻意的：**能被单测的是语义结论，不是像素**，所以层号、行预算、裁剪集、自环记账全留在 Java 里断言，而盒高字号交给唯一真正排版它的那一方，见方案 §8.13。

画布窄到放不下整图时**优先保字不保图**：表级图缩放停在 10px 可读标签的下限上、从链路起点开始摆，被裁掉的那一头拖动画布能看到，右下角同时写明"画布 WxH 放不下 N 个节点"；字段级图的下限是 `0.8` 倍（12px 的字乘 0.8 = 9.6px，再小就只是"有个形状"），缩放走 `transform: scale()`——放大是把这个盒子撑大，不是把字压小，所以任何倍率下盒子里的字都是同一套 CSS 排版。为什么这么取舍，见方案 §8.9、§8.13。

字段级血缘是**一张表一个盒子、盒子里一行一个字段、线连字段行不连盒子**：CTE / 子查询 / UNNEST 折掉的中间关系不再只藏在边的 `hops` 里，而是摊成虚线暗盒画在上下游之间，一条逻辑边拆成几段，点任意一段仍然打开原边的逐跳证据。**交互两步走**：悬停一行只在原地把整条通路染色（不发请求、一个元素都不挪位），点一行才选中并在**画布右上角钉出动作条**（可拖走）：`只看这一列`（沿边收出祖先∪后代闭包、盒子里锥外的行藏掉、剩下的纵向重排，横向一层一列一律不动）/ `列出字段`（右栏出该实体全量清单，宽表在这里看得全）/ `复位`，`×` 或点画布空白把染色和动作条一起撤掉；点盒顶的表名则以那张表为中心重新展开。783 列那种宽表不再拒绝渲染：每盒只画有边的前若干列，剩下的折成一行"还有 N 列未画"，超出登记上限的列数与因两端没画上而暂时缺的段数一起由 `metaInfo.warning` 说清。**不带字段进视图时先讲一个故事**：自动挑一条"既有人喂它、又往下传"的链路（聚合 > 表达式 > 直通）染色选中，标题跟着换成那一列，图上亮的永远是标题说的。**画布左上角有定位搜索框**：候选就是画布上真有的表盒与字段行，字段命中即选中并染色，表盒命中滚进视野闪一下——盒子多了不用肉眼找。**表级关系也不缺位**：两表之间只有表级链路、没有任何字段级血缘（join 键只进 ON/WHERE 的表最典型）时，两盒各出一行 `RelationRows` 并以灰虚线相连，点虚线看是哪条语句——字段视图不再让这种表对凭空消失。这种挂点**不算字段**：标题里的"N 字段行"只数真列，挂点与表级关系对数由 `metaInfo.tableRelRows` / `tableRelSegments` 分开报，图例只在真的有挂点时才讲它；预算把某一端那一行挤掉时虚线退一步接在盒子上，表级关系不因裁剪而静默消失。取舍与实测见方案 §8.10–§8.16。
...
只想要接口不要页面的话：`GET /api/overview`、`/api/tables`、`/api/graph/table`、`/api/graph/column`、`/api/table/{t}/columns`、`/api/edge/column`、`/api/issues` 全部只读，`POST /api/parse` 收一段 SQL 返回试解析结果（不落快照），`POST /api/scan` 是唯一会换快照的写操作（可带 `{"dir":"..."}`）。字段级另有一条 `GET/POST /api/sqlflow/graph`：**数据模型 + 证据 + 落位结论（网格）一次给全**（页面画的就是它，盒子里不含 `x/y/width/height`，有一条 Java 反向断言盯着），`POST` 那份与快照走同一个装配器、同一份口径；`/api/graph/column` 与 `/api/edge/column` 已标 `@Deprecated`——仍按契约保留，但前端已经不再打它们。契约与字段口径写在方案 §7.3。
...
src/main/java/com/bigdata/lineage/
  ...
  web/sqlflow/                                     字段级血缘的对方形状：数据模型 + 关系 + 网格落位（只给 `layer/slot`，不给像素，见方案 §8.13）
  tools/SqlDirLineageTool.java                     目录批量解析 CLI
src/main/resources/static/                         前端：无框架、无打包，ES module 直出；字段级图是 DOM 盒 + 一层 SVG，不碰 cytoscape
